...
router.put(
  ...
  [
    ...
    body("availabilityRange").optional().isObject(),
    body("availabilityRange.startDate").optional({ checkFalsy: true }).isString(),
    body("availabilityRange.endDate").optional({ checkFalsy: true }).isString(),
    body("availabilityRange.excludedWeekdays").optional().isArray(),

    body("dailyTimeRanges").optional().isArray(),
    body("dailyTimeRanges.*.start").optional({ checkFalsy: true }).isString(),
    body("dailyTimeRanges.*.end").optional({ checkFalsy: true }).isString(),
  ...
  async (req, res) => {
    try {
      ...
      delete updated.verificationStatus;
      delete updated.licenseNumber;

      if (
        Array.isArray(updated.dailyTimeRanges) &&
        updated.dailyTimeRanges.length > 0
      ) {
        updated.dailyTimeRanges = updated.dailyTimeRanges.filter(
          (item) =>
            item &&
      ...
      if (
        ...
        delete updated.dailyTimeRanges;
      }

      if (
        updated.availabilityRange &&
        !updated.availabilityRange.startDate &&
        !updated.availabilityRange.endDate &&
        (!updated.availabilityRange.excludedWeekdays ||
          updated.availabilityRange.excludedWeekdays.length === 0)
      ) {
        delete updated.availabilityRange;
      }
  ...
  async (req, res) => {
    try {
      ...
      const totalRevenue = paidAppointment.reduce(
        (sum, apt) => sum + (apt.consultationFees || apt.fees || doctor.fees || 0),
        0
      );
